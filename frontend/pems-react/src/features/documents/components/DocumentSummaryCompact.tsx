...
export function DocumentSummaryCompact({ total, draft, published, archived }: Props) {
  return (
    <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-xs sm:text-sm font-medium text-slate-600">
      ...
      <span>Draft <span className="font-bold text-amber-600">{draft}</span></span>
      <span className="text-slate-300">•</span>
      <span>Published <span className="font-bold text-emerald-600">{published}</span></span>
      <span className="text-slate-300">•</span>
      <span>Archived <span className="font-bold text-slate-600">{archived}</span></span>
    </div>
  );
}
