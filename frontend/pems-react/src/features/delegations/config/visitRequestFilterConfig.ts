...
export function getVisitRequestFilterConfig({
  ...
  if (isVisitor) {
    return {
      ...
      statusOptions: [
        ...
        { value: 'PENDING_CONTACT_CONFIRMATION', label: t ? t('visitRequestV2:list.statusOptions.awaitingConfirmation.label') : 'Chờ đầu mối xác nhận', description: t ? t('visitRequestV2:list.statusOptions.awaitingConfirmation.description') : 'Cơ sở đang chờ đầu mối đoàn khách xác nhận qua link email', requestStatus: 'PENDING_CONTACT_CONFIRMATION' },
        ...
        { value: 'APPROVED', label: t ? t('visitRequestV2:list.statusOptions.approved.label') : 'Đã duyệt', description: t ? t('visitRequestV2:list.statusOptions.approved.description') : 'Đơn đã được duyệt (bao gồm đang chuẩn bị, đang diễn ra hoặc đã hoàn tất)', requestStatus: 'APPROVED' },
        ...
        { value: 'DURING_VISIT', label: t ? t('visitRequestV2:list.statusOptions.duringVisit.label') : 'Đang tiếp khách', description: t ? t('visitRequestV2:list.statusOptions.duringVisit.description') : 'Đoàn đang trong quá trình thăm viếng tại Campus', campusStatus: 'DURING_VISIT' },
        { value: 'CLOSED', label: t ? t('visitRequestV2:list.statusOptions.closed.label') : 'Đã đóng đoàn', description: t ? t('visitRequestV2:list.statusOptions.closed.description') : 'Đoàn đã hoàn tất toàn bộ chuyến thăm và thủ tục', campusStatus: 'CLOSED' },
        { value: 'REJECTED', label: t ? t('visitRequestV2:list.statusOptions.rejected.label') : 'Từ chối', description: t ? t('visitRequestV2:list.statusOptions.rejected.description') : 'Đơn đã bị từ chối', requestStatus: 'REJECTED' },
  ...
  if (isHO) {
    return {
      ...
      statusOptions: [
        ...
        { value: 'WAITING_CONTACT_CONFIRMATION', label: 'Chờ đầu mối xác nhận', description: 'Cơ sở đang chờ đầu mối đoàn khách xác nhận qua link email', campusStatus: 'WAITING_CONTACT_CONFIRMATION' },
        ...
        { value: 'APPROVED', label: 'Đã duyệt', description: 'Các đơn mà mọi cơ sở đã xử lý xong và có cơ sở tiếp nhận', requestStatus: 'APPROVED' },
        // Riêng cho HO theo dõi vận hành: lọc đơn liên cơ sở còn cơ sở chưa xong (dữ liệu vẫn
        // PARTIALLY_APPROVED) — badge của các đơn này giờ hiện "Chờ duyệt" như mọi đơn khác
        // (không còn chữ riêng), phần thay đổi mới thì báo bằng icon chuông cạnh STT.
        { value: 'PARTIALLY_APPROVED', label: 'Còn cơ sở chưa xử lý xong', description: 'Đơn liên cơ sở có cơ sở đã tiếp nhận nhưng vẫn còn cơ sở chờ xử lý hoặc bị từ chối', requestStatus: 'PARTIALLY_APPROVED' },
        { value: 'ASSIGNED', label: 'Đã phân công người phụ trách', description: 'Các cơ sở đã duyệt và có host phụ trách', campusStatus: 'ASSIGNED' },
        ...
        { value: 'REJECTED', label: 'Đã bị từ chối', description: 'Các cơ sở/đơn đã bị từ chối', campusStatus: 'REJECTED' },
  ...
  if (isStaffLeader) {
    return {
      ...
      statusOptions: [
        { value: '', label: 'Tất cả trạng thái' },
        { value: 'WAITING_CONTACT_CONFIRMATION', label: 'Chờ đầu mối xác nhận', description: 'Cơ sở đang chờ đầu mối đoàn khách xác nhận qua link email', campusStatus: 'WAITING_CONTACT_CONFIRMATION' },
        { value: 'PENDING_APPROVAL', label: 'Chờ xử lý tại cơ sở', description: 'Các cơ sở thuộc campus của bạn đang chờ duyệt & gán host', campusStatus: 'WAITING_REQUEST_APPROVAL' },
        { value: 'ASSIGNED', label: 'Đã phân công người phụ trách', description: 'Các cơ sở đã duyệt và có host phụ trách, chưa bắt đầu chuẩn bị', campusStatus: 'ASSIGNED' },
        ...
        { value: 'AFTER_VISIT', label: 'Chờ đóng đoàn', description: 'Bao gồm các đoàn đã kết thúc chuyến thăm và chờ hoàn tất thủ tục đóng đoàn', campusStatus: 'AFTER_VISIT' },
        { value: 'CLOSED', label: 'Đã đóng đoàn', description: 'Bao gồm các đoàn đã hoàn tất toàn bộ quy trình', campusStatus: 'CLOSED' },
        { value: 'REJECTED', label: 'Đã bị từ chối', description: 'Các cơ sở bạn đã từ chối tiếp nhận', campusStatus: 'REJECTED' },
        { value: 'CANCELLED_ANY', label: 'Đã hủy', description: 'Bao gồm các đơn đã bị hủy bỏ', cancelledOnly: true },
      ],
    };
  }
  ...
  if (isRegularStaff && activeTab === 'responsible') {
    return {
      ...
      statusOptions: [
        { value: '', label: 'Tất cả trạng thái' },
        { value: 'ASSIGNED', label: 'Đã phân công người phụ trách', description: 'Các đoàn đã phân công bạn làm Host nhưng chưa bắt đầu', campusStatus: 'ASSIGNED' },
        ...
        { value: 'DURING_VISIT', label: 'Đang tiếp khách', description: 'Các đoàn đang trong thời gian diễn ra', campusStatus: 'DURING_VISIT' },
  ...
  return {
    ...
    statusOptions: [
        ...
        { value: 'DURING_VISIT', label: 'Đang tiếp khách', description: 'Đoàn đang được tiếp đón', campusStatus: 'DURING_VISIT' },
        { value: 'CLOSED', label: 'Đã đóng đoàn', description: 'Chuyến thăm đã hoàn tất', campusStatus: 'CLOSED' },
        { value: 'REJECTED', label: 'Đã bị từ chối', description: 'Đơn bị từ chối', requestStatus: 'REJECTED' },
