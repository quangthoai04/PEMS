...
export function VisitRequestManagement() {
  ...
  // non-host participant: regular Staff, Dept, Student. HO, Staff Leader, Visitor and Admin
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const params: Record<string, unknown> = {
        tab: isVisitor ? 'responsible' : activeTab,
        page: currentPage,
        pageSize,
      };
      ...
      if (appliedFilters.status) {
        const option = STATUS_FILTER_OPTIONS.find((o) => o.value === appliedFilters.status) as any;
      ...
      if (appliedFilters.visitScopes.length > 0) params.visitScopes = appliedFilters.visitScopes.join(',');
  ...
  const submitReject = async () => {
    ...
    try {
      ...
      const msg = e?.response?.data?.message || e?.response?.data?.title || e?.message || 'Lỗi không xác định';
      setReject((s) => ({ ...s, submitting: false, error: `Không thể từ chối đơn. ${msg}` }));
    }
  };
  ...
  const renderBadges = (row: Row) => {
    ...
    if (activeTab === 'attending') {
      badges.push(chip('att', 'Đơn mời tham dự', 'bg-purple-50 text-purple-700 border-purple-200'));
  ...
  const getStatusBadge = (statusText: string) => {
    ...
    switch (statusText) {
      case 'Chờ duyệt':
      case 'Chờ phân công':
        return <span className={`${base} bg-yellow-50 text-yellow-700 border-yellow-200`}>{statusText}</span>;
      case 'Đã duyệt':
        return <span className={`${base} bg-cyan-50 text-cyan-700 border-cyan-200`}>Đã duyệt</span>;
      case 'Trước tiếp khách':
        return <span className={`${base} bg-blue-50 text-blue-700 border-blue-200`}>Trước tiếp khách</span>;
      case 'Trong tiếp khách':
        return <span className={`${base} bg-green-50 text-green-700 border-green-200`}>Trong tiếp khách</span>;
      case 'Chờ đóng đoàn':
        return <span className={`${base} bg-orange-50 text-orange-700 border-orange-200`}>Chờ đóng đoàn</span>;
      case 'Đã đóng đoàn':
        return <span className={`${base} bg-slate-100 text-slate-700 border-slate-300`}>Đã đóng đoàn</span>;
      case 'Từ chối':
        return <span className={`${base} bg-red-50 text-red-700 border-red-200`}>Từ chối</span>;
      case 'Đã hủy':
        return <span className={`${base} bg-gray-100 text-gray-600 border-gray-200`}>Đã hủy</span>;
      default:
        return <span className={`${base} bg-gray-100 text-gray-700 border-gray-200`}>{statusText}</span>;
    }
  };
  ...
  const renderRowActions = (row: Row) => {
    ...
    return (
      <div className="mx-auto grid w-[108px] grid-cols-3 gap-2 place-items-center">
        ...
        {/* Slot 2: Approve / Transfer / Reason */}
        {can('HO_APPROVE') ? (
          ...
          <ActionIconButton title="Chuyển host phụ trách" tone="blue" icon={<UserCog className="h-5 w-5" />}
    ...
    <div className="w-full max-w-[1320px] mx-auto p-4 sm:p-6 lg:p-8 flex flex-col space-y-6 pb-12 animate-in fade-in duration-300">
      ...
      {showTabs && (
        <div className="flex w-full sm:w-max items-center gap-1 rounded-xl bg-slate-100 p-1">
          {([
            { key: 'responsible' as Tab, label: 'Đơn phụ trách' },
            { key: 'attending' as Tab, label: 'Đơn mời tham dự' },
          ]).map((t) => (
            <button
              key={t.key}
              onClick={() => { if (activeTab !== t.key) { setActiveTab(t.key); setCurrentPage(1); } }}
      ...
      <div className="w-full rounded-2xl border border-slate-200 bg-white p-4 shadow-sm overflow-visible">
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-[minmax(280px,1fr)_180px_170px_210px_112px_44px] xl:items-end w-full">
          ...
          <div className="relative min-w-0 w-full">
            ...
            <button onClick={() => setIsStatusFilterOpen(!isStatusFilterOpen)} className="flex h-11 w-full min-w-0 items-center justify-between rounded-xl border border-slate-300 bg-white px-3 text-sm font-semibold text-slate-700 outline-none transition-colors focus:border-[#004c91]">
              <span className="min-w-0 truncate">{STATUS_FILTER_OPTIONS.find((o) => o.value === draftFilters.status)?.label ?? 'Tất cả trạng thái'}</span>
              ...
              <>
                ...
                <div className="absolute left-0 top-full z-30 mt-1 w-full min-w-[210px] rounded-xl border border-slate-200 bg-white py-1 shadow-lg max-h-72 overflow-y-auto">
                  {STATUS_FILTER_OPTIONS.map((option) => (
                    <div key={option.value} className={`px-3 py-2 text-sm cursor-pointer hover:bg-slate-50 flex items-center ${draftFilters.status === option.value ? 'bg-blue-50 text-[#004c91] font-bold' : 'text-gray-700 font-medium'}`}
          ...
          <div className="relative min-w-0 w-full">
            ...
            <button onClick={() => setIsTypeFilterOpen(!isTypeFilterOpen)} className="flex h-11 w-full min-w-0 items-center justify-between rounded-xl border border-slate-300 bg-white px-3 text-sm font-semibold text-slate-700 outline-none transition-colors focus:border-[#004c91]">
              <span className="min-w-0 truncate">{draftFilters.visitScopes.length === 0 ? 'Tất cả phạm vi' : draftFilters.visitScopes.length === 1 ? (VISIT_SCOPE_OPTIONS.find((x) => x.value === draftFilters.visitScopes[0])?.label ?? '1 phạm vi') : `${draftFilters.visitScopes.length} phạm vi`}</span>
              <ChevronDown className="w-4 h-4 text-gray-500 flex-shrink-0 ml-2 pointer-events-none" />
            </button>
            {isTypeFilterOpen && (
              <>
          ...
          <button onClick={handleApplyFilters} className="inline-flex h-11 w-full items-center justify-center rounded-xl bg-[#004c91] px-4 text-sm font-bold text-white transition-colors hover:bg-[#003b70] whitespace-nowrap">Áp dụng</button>
      ...
      <div className="w-full overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm flex flex-col">
        ...
        <div className="hidden lg:block w-full">
          <div className="grid grid-cols-[52px_minmax(0,1fr)_210px_150px_140px] bg-[#004c91] text-white">
            ...
            <div className="p-3 text-[12px] font-bold text-left uppercase tracking-wider">Lịch tiếp</div>
          ...
          <div className="flex flex-col">
            {isLoading ? (
              ...
              <div key={row.id} className={`grid grid-cols-[52px_minmax(0,1fr)_210px_150px_140px] items-center min-h-[78px] border-b border-slate-200/70 transition-colors duration-150 cursor-pointer ${index % 2 === 0 ? 'bg-white' : 'bg-slate-50'} hover:bg-blue-50 group`} onClick={() => handleView(row)}>
                ...
                <div className="py-3 px-3 min-w-0 flex flex-col justify-center pr-4">
                  ...
                  {!isHO && (
                    <p className="text-xs font-medium text-slate-600 mt-0.5 truncate">
                      <span className="text-slate-400">Host:</span> {row.host || (row.requestStatus === 'APPROVED' && isVisitor ? 'Đang phân công' : '-')}
                ...
                <div className="py-3 px-3 flex flex-col items-center justify-center gap-1">{getStatusBadge(row.statusText)}</div>
          ...
          {isLoading ? (
            ...
            <div key={row.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm hover:border-[#004c91]/30 transition-colors cursor-pointer" onClick={() => handleView(row)}>
              <div className="flex items-start justify-between gap-3 mb-2">
                ...
                <div className="flex-shrink-0">{getStatusBadge(row.statusText)}</div>
              </div>
              {renderBadges(row)}
          ...
          <div className="p-6 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-4 bg-gray-50/50">
            <div className="flex items-center gap-2">
              ...
              <div className="relative">
                <select value={pageSize} onChange={(e) => { setPageSize(Number(e.target.value)); setCurrentPage(1); }} className="px-3 py-1.5 pr-8 rounded-lg border border-gray-200 text-sm font-bold text-gray-700 bg-white focus:outline-none appearance-none min-w-[70px] text-left">
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => setCurrentPage((p) => Math.max(1, p - 1))} disabled={currentPage === 1} className="p-2 rounded-lg border border-gray-200 text-gray-500 hover:text-[#004c91] hover:border-[#004c91] hover:bg-blue-50 transition-all disabled:opacity-50 disabled:cursor-not-allowed outline-none"><ChevronLeft className="w-4 h-4" /></button>
              <div className="flex items-center gap-1">
                {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                  <button key={page} onClick={() => setCurrentPage(page)} className={`w-8 h-8 rounded-lg text-sm font-bold transition-all outline-none ${currentPage === page ? 'bg-[#004c91] text-white shadow-sm' : 'text-gray-600 hover:bg-gray-200'}`}>{page}</button>
                ))}
              </div>
              <button onClick={() => setCurrentPage((p) => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className="p-2 rounded-lg border border-gray-200 text-gray-500 hover:text-[#004c91] hover:border-[#004c91] hover:bg-blue-50 transition-all disabled:opacity-50 disabled:cursor-not-allowed outline-none"><ChevronRight className="w-4 h-4" /></button>
            </div>
          </div>
        )}
      </div>
      ...
      {assign.open && assign.row && (
        <AssignHostModal
          ...
          onConfirmed={() => { setAssign({ open: false, row: null, mode: 'approve' }); fetchData(); }}
        />
      )}
    </div>
  );
