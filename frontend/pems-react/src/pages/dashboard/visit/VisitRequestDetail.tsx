/**
 * Trang VisitRequestDetail
 * Bảng lưu trữ khai báo, xử lý tài liệu yêu cầu chuyến tham quan cụ thể.
 */
...
export function VisitRequestDetail() {
  ...
  const [actionTime, setActionTime] = useState<string | null>(() => inMemoryTaskStore[`visitRequest_${id}_${type}_actionTime`] || null);
  ...
  useEffect(() => {
    inMemoryTaskStore[`visitRequest_${id}_${type}_actionStatus`] = taskActionStatus;
  }, [taskActionStatus, id, type]);

  useEffect(() => {
    inMemoryTaskStore[`visitRequest_${id}_${type}_proposedBy`] = proposedBy;
  }, [proposedBy, id, type]);

  useEffect(() => {
    inMemoryTaskStore[`visitRequest_${id}_${type}_proposedTime`] = proposedTime;
  }, [proposedTime, id, type]);

  useEffect(() => {
    inMemoryTaskStore[`visitRequest_${id}_${type}_proposedContent`] = proposedContent;
  }, [proposedContent, id, type]);

  useEffect(() => {
    if (actionTime) {
      inMemoryTaskStore[`visitRequest_${id}_${type}_actionTime`] = actionTime;
    }
  }, [actionTime, id, type]);
  ...
  // Mock data
  const taskStatus = "Đang chờ";
  const coordinatorName = "Trần Thị IC";
  const supporterName = isStaff ? user?.name : "Trưởng phòng";
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8 max-w-[95%] mx-auto pb-24 relative animate-in fade-in duration-500">
      ...
      <div className="mb-4 flex items-center text-sm font-medium text-gray-500">
        ...
        <span className="mx-2">/</span>
        <button
          onClick={() => navigate(`/dashboard/visit/process/${id}`)}
          className="hover:text-[#004c91] transition-colors outline-none"
        >
          ...
          <div>
            <h1 className="text-3xl lg:text-4xl font-black text-[#004c91] tracking-tight uppercase flex items-center gap-4">
              ...
              <span className={`inline-flex items-center px-4 py-1.5 rounded-full text-sm font-bold shadow-sm ${
                ...
                {taskActionStatus === 'pending' && <div className="w-2 h-2 rounded-full bg-orange-500 mr-2 animate-pulse"></div>}
                {taskActionStatus === 'pending' ? 'Đang xử lý' : 'Hoàn thành'}
              </span>
            </h1>
          </div>
        </div>
      ...
      <div className="flex flex-col w-full max-w-4xl mx-auto gap-8">
        ...
        <div className="bg-white rounded-[2rem] shadow-[0_8px_30px_-4px_rgba(0,0,0,0.05)] border border-gray-100 p-0 flex flex-col relative overflow-hidden group/card hover:shadow-[0_12px_40px_-4px_rgba(0,76,145,0.08)] transition-shadow duration-500">
          ...
          <div className="flex items-center gap-4 bg-[#004c91] p-8 pb-6 relative z-10">
            ...
            <div>
              ...
              <p className="text-sm font-medium text-blue-100 mt-1">Yêu cầu được giao</p>
            </div>
          </div>
          ...
          <div className="flex-1 space-y-6 relative z-10 p-8 pt-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="p-4 bg-gray-50/80 rounded-2xl border border-gray-100 hover:border-[#004c91]/30 hover:shadow-md hover:bg-white transition-all cursor-default">
                ...
                <div className="text-sm font-black text-[#004c91]">{coordinatorName}</div>
              </div>
              ...
              <div className="col-span-2 p-4 bg-gray-50/80 rounded-2xl border border-gray-100 hover:border-[#f37021]/30 hover:shadow-md hover:bg-white transition-all cursor-default">
                ...
                <div className="text-base font-black text-[#004c91] border-l-4 border-[#f37021] pl-3 py-1 bg-transparent">
                  Đoàn đối tác từ Đại học Tokyo, Nhật Bản
                </div>
              </div>
            ...
            {!isProposing && taskActionStatus === 'pending' && ['DEPARTMENT', 'STAFF'].includes(user?.role?.toUpperCase()) && (
              <div className="flex justify-end pt-2">
                <button 
                  ...
                  className="px-6 py-2.5 rounded-xl bg-orange-50 text-[#f37021] font-bold hover:bg-orange-100 transition-colors flex items-center gap-2 border border-orange-200"
                >
          ...
          {taskActionStatus === 'pending' && !isProposing && (
            <div className="flex gap-4 p-8 pt-6 border-t border-gray-100 relative z-10 bg-gray-50/50">
              ...
              <button 
                ...
                className="flex-1 py-4 rounded-2xl bg-[#004c91] text-white font-black uppercase tracking-wider hover:bg-[#003b73] transition-all duration-300 shadow-lg shadow-[#004c91]/20 outline-none hover:shadow-[0_8px_25px_rgba(0,76,145,0.3)] active:scale-[0.98] border border-blue-600">
                Xác nhận
              </button>
            </div>
          )}
        ...
        {/* RIGHT COLUMN: DIGITAL AGREEMENT SIGNING */}
        {taskActionStatus === 'confirmed' && (
            ...
            <div className="bg-white rounded-2xl border border-slate-200/85 shadow-md p-6 md:p-8 font-sans w-full max-w-4xl mx-auto space-y-6 relative overflow-hidden mb-8">
              ...
              <div className="text-center space-y-1.5 relative z-10 pt-2">
                ...
                <p className="text-[11px] font-semibold text-slate-505 italic">
                  (V/v: Thiết lập trách nhiệm và phân công công việc đón tiếp phái đoàn)
                </p>
              </div>
              ...
              <div className="space-y-4 text-xs text-slate-750 font-normal leading-relaxed relative z-10 font-sans">
                <p className="text-justify px-2">
                  Căn cứ quyết định phân công nhiệm vụ đón tiếp ngày hôm nay. Các bên liên quan đã được thông báo và đồng ý thực hiện đúng trách nhiệm công việc được giao tại campus Đại học FPT Hòa Lạc. Các bên tham gia trực tiếp xác nhận rõ ràng bằng chữ ký số:
                </p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-slate-50/80 p-5 rounded-2xl border border-slate-200/50">
                  ...
                  <div className="space-y-2 md:pl-2">
                    ...
                    <div className="flex items-center gap-1.5">• <strong>Nhiệm vụ</strong>: <span className="text-slate-850 font-bold">Thực hiện công tác hướng dẫn</span></div>
                  </div>
                </div>
              </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="bg-slate-50/50 rounded-xl p-4 border border-slate-200 shadow-sm flex flex-col gap-4">
                      <div className="mb-auto">
                        ...
                        <p className="text-[11px] text-slate-500 italic p-2.5 bg-slate-100 rounded-xl border border-slate-200">Đã hoàn thành bàn giao và trả lại tài sản.</p>
                      </div>
                      <div 
                        className={`group relative p-3 rounded-xl border-2 transition-all shadow-sm cursor-pointer mt-auto ${
      ...
      <div className="mt-8 flex justify-center lg:justify-end border-t border-gray-200 pt-6 relative">
        ...
        >
          ...
          {isTaskComplete ? 'Hoàn thành yêu cầu' : 'Chưa đủ điều kiện'}
        </button>
      </div>
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <textarea 
                  ...
                  placeholder="Vui lòng nhập lý do từ chối yêu cầu này để hệ thống ghi nhận..."
                />
              </div>
            </div>
          ...
          <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
            ...
            <div className="p-6">
              <p className="text-gray-700 text-[15px] font-medium leading-relaxed">
                Bạn có chắc chắn muốn gửi đề xuất thay đổi thời gian và nội dung yêu cầu này không? 
                <br/><br/>
                Yêu cầu đề xuất sẽ được gửi lên hệ thống và chuyển trạng thái yêu cầu thành <strong>Chờ xác nhận</strong>.
              </p>
            </div>
            <div className="p-6 bg-gray-50 border-t border-gray-100 flex justify-end gap-3">
              <button 
