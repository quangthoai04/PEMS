...
{
    private static (CreateAccountCommandHandler handler, TestApplicationDbContext db, FakeSystemEmailDispatcher dispatcher)
        Build(EmailDeliveryResult emailResult)
    {
        ...
        var dispatcher = new FakeSystemEmailDispatcher { Outcome = emailResult };
    ...
    {
        var (handler, db, _) = Build(EmailDeliveryResult.Skipped("SMTP_DISABLED"));
    ...
    {
        var (handler, db, _) = Build(EmailDeliveryResult.Failed("SMTP_SEND_FAILED", "Email delivery failed."));
    ...
    {
        var (handler, _, dispatcher) = Build(EmailDeliveryResult.Sent());

        var res = await handler.Handle(StudentCmd(), CancellationToken.None);

        Assert.Equal("SENT", res.EmailNotificationStatus);

        // The account-creation path no longer has an email of its own: it names the SAME template the
        // resend and edit paths name, and the confirm link arrives only as a trusted block.
        var sent = dispatcher.Single(SystemEmailTemplates.AccountEmailConfirmation);
        Assert.Equal("new.student@fpt.edu.vn", sent.To.Email);
        Assert.Contains("confirm-email?token=raw-token", sent.TrustedBlocks![EmailTrustedBlocks.ActionBlock]);
        Assert.Equal(
    ...
    {
        var (handler, db, _) = Build(EmailDeliveryResult.Sent());
