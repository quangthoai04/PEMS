...
public sealed class RequestDetailV2Tests
{
    ...
    public async Task V1_returns_global_form_byte_identical()
    {
        ...
        var (_, items) = await Seed(db, FormSchemaVersions.Legacy, new[] { Campus1 }, mixed: false);
        ...
        Assert.Equal("+8491", dto.ContactPersonPhone);
        // Registrant identity is request-level in both versions.
