...
export function PartnersSection() {
  // Trạng thái lưu trữ danh sách đối tác
  const [partners, setPartners] = useState<any[]>(partnersData);
  ...
  useEffect(() => {
    const fetchPartners = async () => {
      try {
        const apiUrl = import.meta.env.VITE_API_URL || 'http://localhost:5265';
        const response = await fetch(`${apiUrl}/api/home/partners`);
        if (!response.ok) {
          throw new Error('Lỗi tải dữ liệu');
        }
        const data = await response.json();
        // Nếu cơ sở dữ liệu có đối tác đã duyệt, ta sử dụng dữ liệu động
        if (data && data.length > 0) {
          // Chuẩn hóa đường dẫn ảnh nếu cần hoặc lấy trực tiếp URL từ server
          const formattedPartners = data.map((p: any) => ({
            name: p.name || 'Đối tác',
            logo: p.logo && typeof p.logo === 'string'
              ? (p.logo.startsWith('http') || p.logo.startsWith('data:') ? p.logo : `${apiUrl}${p.logo}`)
              : 'https://images.unsplash.com/photo-1541339907198-e08756dedf3f?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80'
          }));
          setPartners(formattedPartners);
        }
      } catch (error) {
        console.warn("API Offline hoặc chưa có đối tác được duyệt trong DB, sử dụng logo mặc định:", error);
      }
    };
    fetchPartners();
  }, []);

  const activePartners = partners && partners.length > 0 ? partners : partnersData;
  ...
  const currentPartners = activePartners.slice(
