...
export function NewsSection() {
  // Trạng thái lưu trữ tin tức
  const [news, setNews] = useState<any[]>([]);
  const [loading, setLoading] = useState<boolean>(true);

  // Thực hiện call API từ Backend
  useEffect(() => {
    const fetchNews = async () => {
      try {
        ...
        console.warn("API Offline hoặc lỗi, sử dụng dữ liệu mockup cục bộ:", error);
        setNews(mockNews);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <section id="news" className="py-24 bg-slate-50/80 border-y border-gray-100 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {loading ? (
          // Hiệu ứng Loading Skeleton đẹp mắt
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[1, 2, 3].map((n) => (
              <div key={n} className="bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm animate-pulse h-96 flex flex-col">
