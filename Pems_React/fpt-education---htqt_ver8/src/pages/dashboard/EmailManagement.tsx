...
const mockEmailData = [
  ...
  { id: 7, name: 'Đề xuất hợp tác dự án', subject: 'Đề xuất nội dung hợp tác nghiên cứu chung', desc: 'Gửi các ý tưởng hoặc văn bản đề xuất triển khai các dự án thực tế với đối tác.', creator: 'Nguyễn Văn B', campus: 'Hồ Chí Minh', date: '07/05/2024', status: 'Sử dụng' },
  { id: 8, name: 'Tài liệu giới thiệu', subject: 'Tài liệu giới thiệu về Đại học FPT (Brochure)', desc: 'Gửi thông tin tổng quan, thông tin về các chuyên ngành và năng lực của nhà trường.', creator: 'Nguyễn Văn C', campus: 'Cần Thơ', date: '08/05/2024', status: 'Không sử dụng' },
...
export function EmailManagement() {
  ...
  const initialTab = queryParams.get('tab') === 'sent' ? 'Danh sách email đã gửi' : 'Mẫu email';
    ...
    return (
      <div className="p-8 h-full flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-500">Trang này không dành cho tài khoản Student.</p>
        </div>
      </div>
    );
  }
  ...
  let filteredSentData = sentData.filter(item => {
    const matchSearch = item.program.toLowerCase().includes(searchQuerySent.toLowerCase()) || 
                       ...
                       item.sender.toLowerCase().includes(searchQuerySent.toLowerCase());
              ...
              <table className="w-full border-collapse min-w-[1000px]">
                <thead>
                  <tr className="bg-[#004c91] text-white text-[12px] tracking-wide uppercase whitespace-nowrap">
                    ...
                    <th className="p-3 font-bold w-[110px] text-center">TRẠNG THÁI</th>
                    <th className="p-3 font-bold w-[130px] text-center">NGƯỜI TẠO</th>
                    <th 
                  ...
                  {currentItems.map((item, index) => (
                    <tr key={item.id} className="hover:bg-gray-50/80 transition-colors group">
                      ...
                      <td className="p-3 align-middle whitespace-nowrap text-center">
                        ...
                        {userRole === 'HO' && item.campus && (
                          <div className="text-[12px] text-gray-500 mt-0.5">{item.campus}</div>
                        )}
                      </td>
          ...
          <div className="flex items-center flex-wrap gap-3 mb-6">
            <div className="relative flex-1 min-w-[200px] max-w-sm">
              ...
              <input 
                ...
                onChange={(e) => setSearchQuerySent(e.target.value)}
            ...
            >
              ...
              <option value="Tân sinh viên K19">Tân sinh viên K19</option>
              <option value="Học bổng 2024">Học bổng 2024</option>
              <option value="Kỳ Fall 2024">Kỳ Fall 2024</option>
              <option value="Sự kiện sinh viên">Sự kiện sinh viên</option>
            ...
            <select 
              ...
              onChange={(e) => setStatusFilterSent(e.target.value)}
              className="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none hover:border-[#004c91] hover:text-[#004c91] focus:border-[#004c91] text-gray-600 bg-white font-medium shadow-sm transition-colors cursor-pointer outline-none"
            >
              <option value="">Tất cả trạng thái</option>
            ...
            {userRole === 'HO' && (
              <select 
                value={campusFilterSent}
                onChange={(e) => setCampusFilterSent(e.target.value)}
                className="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none hover:border-[#004c91] hover:text-[#004c91] focus:border-[#004c91] text-gray-600 bg-white font-medium shadow-sm transition-colors cursor-pointer outline-none"
              >
                <option value="">Tất cả cơ sở</option>
                <option value="Hà Nội">Hà Nội</option>
            ...
            <div className="overflow-x-auto">
              <table className="w-full border-collapse min-w-[1000px]">
                <thead>
                  <tr className="bg-[#004c91] text-white text-[12px] tracking-wide uppercase whitespace-nowrap">
                    <th className="p-3 font-bold w-[70px] text-center">STT</th>
                    <th className="p-3 font-bold text-left pl-6">TÊN CHƯƠNG TRÌNH</th>
                    <th className="p-3 font-bold text-left pl-6">TIÊU ĐỀ</th>
                    <th className="p-3 font-bold text-center">NGƯỜI GỬI</th>
                    <th 
                      className="p-3 font-bold w-[160px] text-center cursor-pointer hover:bg-[#003a70] bg-[#004c91] text-white transition-colors select-none group"
                      onClick={() => {
                    ...
                    <tr key={item.id} className="hover:bg-gray-50/80 transition-colors group">
                      ...
                      <td className="p-3 align-middle font-bold text-[#004c91] text-[14px] text-left pl-6">
                        <div className="flex items-center gap-2">
                          ...
                          <div className="line-clamp-2">{item.program}</div>
                        </div>
                      </td>
                      ...
                      <td className="p-3 align-middle whitespace-nowrap text-center">
                        <div className="font-bold text-gray-700 text-[14px]">{item.sender}</div>
                        {userRole === 'HO' && item.campus && (
                          <div className="text-[12px] text-gray-500 mt-0.5">{item.campus}</div>
                        )}
                      </td>
