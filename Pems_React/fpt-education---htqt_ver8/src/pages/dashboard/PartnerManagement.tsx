...
export function PartnerManagement() {
  ...
  if (!["STAFF", "ADMIN", "HO", "STUDENT"].includes(userRole)) {
    return (
      <div className="p-8 h-full flex items-center justify-center bg-slate-50/50">
  ...
  const filteredData = data.filter((item) => {
    ...
    return matchSearch && matchStatus && matchCountry && matchCampus;
  });
  ...
  const renderActions = (status: string, creator: string, creatorCampus: string) => {
    const viewBtn = (
      <button
        key="view"
  ...
  return (
    <div className="p-8 max-w-[95%] mx-auto pb-12">
      {/* Breadcrumb */}
      ...
      {isStudent ? (
        ...
        <div>
          ...
          <div className="flex flex-wrap items-center gap-3 mb-6">
            ...
            <button className="ml-auto bg-[#f37021] hover:bg-[#d9621a] text-white px-4 py-2 rounded-md text-sm font-bold flex items-center gap-1.5 transition-colors shadow-sm outline-none tracking-wide">
                  ...
                  {paginatedData.length > 0 ? (
                    paginatedData.map((item, index) => (
                      ...
                      >
                        ...
                        <td className="p-3 align-middle whitespace-nowrap">
                          {renderActions(item.status, item.creator, item.campus)}
                        </td>
                      </tr>
                    ))
                  ) : (
        ...
        <div className="flex items-center gap-1.5">
          ...
          <div className="flex items-center gap-1">
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
              <button
                key={p}
                className={`w-9 h-9 rounded-xl font-bold text-sm transition-colors cursor-pointer border ${validPage === p ? "bg-[#004c91] text-white shadow-sm border-[#004c91]" : "text-gray-600 hover:bg-gray-100 border-transparent"}`}
                onClick={() => setPage(p)}
              >
                {p}
              </button>
