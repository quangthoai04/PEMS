...
export function NewsManagement() {
  ...
  const getTypeBadge = (type: string) => {
    if (type === 'News') {
      return <span className="inline-block px-3 py-1 bg-[#eaffe4] text-[#0aa14f] font-black rounded-xl text-[11px] tracking-wider uppercase shadow-sm border border-[#ceefda]">NEWS</span>;
    }
    return <span className="inline-block px-3 py-1 bg-blue-50 text-blue-700 font-bold rounded-xl text-[11px] tracking-wider uppercase shadow-sm border border-blue-100">{type}</span>;
  };
  ...
  const renderActions = (id: number, status: string, creator: string) => {
    ...
    const deleteBtn = <button key="delete" className={`${btnClass} hover:bg-red-50 hover:text-red-600 text-gray-400`} title="Xóa"><Trash2 className="w-[16px] h-[16px]" /></button>;
    const toggleBtn = (
    ...
    if (isAdmin || isHO) {
      switch (status) {
        case 'Đã Duyệt': return <div className="flex items-center justify-center gap-1">{viewBtn}{editBtn}{deleteBtn}{toggleBtn}</div>;
        case 'Chờ Duyệt': return <div className="flex items-center justify-center gap-1">{viewBtn}{editBtn}{acceptBtn}{denyBtn}</div>;
        case 'Từ Chối': return <div className="flex items-center justify-center gap-1">{viewBtn}{deleteBtn}</div>;
        case 'Ẩn': return <div className="flex items-center justify-center gap-1">{viewBtn}{editBtn}{deleteBtn}{toggleBtn}</div>;
        default: return null;
      }
    }
    ...
    if (isStaff) {
      if (creator === user?.name) {
        switch (status) {
          ...
          case 'Từ Chối':
            return <div className="flex items-center justify-center gap-1">{viewBtn}{deleteBtn}</div>;
          default:
            return null;
        }
      } else {
          ...
          <table className="w-full border-collapse min-w-[1000px]">
            <thead>
              <tr className="bg-[#004c91] text-white text-[12px] tracking-wide uppercase text-center">
                <th className="p-3 font-bold w-[90px] whitespace-nowrap">LOẠI TIN</th>
            ...
            <tbody className="divide-y divide-gray-200">
              {currentItems.map(item => (
                <tr key={item.id} className="hover:bg-gray-50/80 transition-colors group text-center">
                  <td className="p-3 align-middle whitespace-nowrap">{getTypeBadge(item.type)}</td>
