    ...
    string? Notes)
{
    ...
    /// canonical recompute (<c>VisitRequestV2Canonical</c>). Processing is not part of an edit.</summary>
    public CampusVisitFormDto ToFormDto() => new(
        ...
        MediaConsentStatus, MediaConsentNote, Notes, Processing: null);
}

/// <summary>The complete per-campus v2 edit payload (pending-edit and resubmit share it).</summary>
public record VisitRequestEditV2Dto(
    int ExpectedRequestRowVersion,
    RegistrantInputV2 Registrant,
    ContactPointDto PrimaryContact,
    ulong? PartnerId,
