...
public sealed class UpdateAccountRoleCommandHandler : IRequestHandler<UpdateAccountRoleCommand, UpdateAccountRoleResponse>
{
    ...
    public async Task<UpdateAccountRoleResponse> Handle(UpdateAccountRoleCommand request, CancellationToken cancellationToken)
    {
        ...
        // ── ADMIN = Global Read + Security Control (ADMIN_ACCOUNT_MANAGEMENT spec §12). Who somebody
        //    is and where they work — role, sub-role, campus, department, MSSV, name, email — is
        //    personnel management, owned by HO / Staff Leader. Refused BEFORE the transaction and the
        //    row lock so a denied request never takes a lock, never reads the target and cannot leave
