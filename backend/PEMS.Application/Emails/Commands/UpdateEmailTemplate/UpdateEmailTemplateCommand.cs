...
/// </para>
/// <para>
/// <b>There is no configuration half any more.</b> This command used to carry contact settings alongside
/// the wording, because whether a body could legally keep or drop <c>{{contactInformationBlock}}</c>
/// depended on a policy stored in another table — so the two had to be saved in one transaction or they
/// could contradict each other. Sender information is ordinary template variables: an administrator adds
/// or removes <c>{{senderName}}</c> the same way they add or remove <c>{{fullName}}</c>, there is nothing
/// else to keep in step, and the whole atomic-save-across-two-concerns problem is gone with it.
/// </para>
/// </summary>
