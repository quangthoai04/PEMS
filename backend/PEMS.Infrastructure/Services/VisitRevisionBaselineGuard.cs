...
internal static class VisitRevisionBaselineGuard
{
    ...
    /// <summary>
    /// The next request revision number, counting the baseline this class may have just STAGED.
    ///
    /// <para>
    /// <see cref="EnsureRequestBaselineAsync"/> deliberately does not flush — the baseline has to
    /// commit inside the caller's own transaction, so a rolled-back edit cannot leave one behind.
    /// The consequence is that a <c>MAX(request_revision)</c> asked of the DATABASE cannot see it:
    /// on a request whose history was empty, the baseline takes revision 1 and the query still
    /// answers "nothing here", so the row that follows is numbered 1 as well and the pair collides
    /// on <c>uq_vrrh_request_revision</c> — a 500 out of the pending-edit and resubmit endpoints.
    /// </para>
    /// <para>
    /// So the maximum is taken over BOTH what is persisted and what this unit of work has already
    /// added. Same intent as before (max + 1); it just now counts everything that will exist when
    /// the transaction commits.
    /// </para>
    /// </summary>
    public static async Task<uint> NextRequestRevisionAsync(
        IApplicationDbContext db, ulong visitRequestId, CancellationToken ct)
    {
        var persisted = await db.VisitRequestRevisionHistories
            .Where(r => r.VisitRequestId == visitRequestId)
            .MaxAsync(r => (uint?)r.RequestRevision, ct) ?? 0u;

        var staged = db.VisitRequestRevisionHistories.Local
            .Where(r => r.VisitRequestId == visitRequestId)
            .Select(r => r.RequestRevision)
            .DefaultIfEmpty(0u)
            .Max();

        return (persisted > staged ? persisted : staged) + 1;
    }
